...
def forgot_password_sms(phone, proxy_dict=None):
    print(f"{Fore.YELLOW}[DEBUG] Trying: {phone}")
    try:
        scraper = cloudscraper.create_scraper()
        if proxy_dict:
            scraper.proxies = proxy_dict
            print(f"{Fore.CYAN}[DEBUG] Using proxy: {proxy_dict}")
        ...
        print(f"{Fore.CYAN}[DEBUG] GET Status: {resp.status_code}")
        ...
        print(f"{Fore.CYAN}[DEBUG] POST Status: {resp.status_code}")
        print(f"{Fore.CYAN}[DEBUG] URL: {resp.url}")
        ...
        try:
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    ...
                    if len(num) >= 9:  # Egypt code 20 + 9 digits = 11
                        numbers.add(num)
        except Exception as e:
...
# ================= WORKER =================
def worker(numbers, results, proxy_list, thread_id, delay=0.5):
...
def main():
    ...
    # ── Threads ──
    try:
        threads_count = int(input(f"{Fore.CYAN}Enter threads (default 1, recommended): ").strip() or 1)
    except:
        threads_count = 1
    
    try:
        delay = float(input(f"{Fore.CYAN}Enter delay between requests (default 1s): ").strip() or 1)
    except:
        delay = 1
    ...
    for i, chunk in enumerate(chunks):
        t = threading.Thread(target=worker, args=(chunk, results, proxy_list, i+1, delay))
        t.start()
        threads.append(t)
